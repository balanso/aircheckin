...
$owner_balance_items = $db->getAll("SELECT * FROM owners_balance WHERE owner_id=?i AND payed_out=0", $owner_id);
$owner_balance       = 0;
...
if (!empty($owner_balance_items)) {
  ...
  if (is_numeric($owner_balance)) {
    ...
    if ($payu_balance && $payu_balance >= $payout_sum) {
      ...
      if (isset($result[1]) && $result[1] == 'SUCCESS') {
        ...
        json_answer(['message'=>'Мы зафиксировали ошибку, продолжаем тестирование сервиса и с вашего позволения переведём вам деньги в ручном режиме. Благодарим за помощь в тестировании!']);
      }
    } else {
  ...
  json_answer('error', 'Balance orders for payout not found');
}
